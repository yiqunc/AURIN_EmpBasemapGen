...
div1 = read.table("VIC\\div1_v5.csv", header=TRUE, na.strings="NA", sep="\t", blank.lines.skip=TRUE, fill=TRUE, colClasses="character")
div2 = read.table("VIC\\div2_v5.csv", header=TRUE, na.strings="NA", sep="\t", blank.lines.skip=TRUE, fill=TRUE, colClasses="character")
div3 = read.table("VIC\\div3_v5.csv", header=TRUE, na.strings="NA", sep="\t", blank.lines.skip=TRUE, fill=TRUE, colClasses="character")
div4 = read.table("VIC\\div4_v5.csv", header=TRUE, na.strings="NA", sep="\t", blank.lines.skip=TRUE, fill=TRUE, colClasses="character")
...
f_datamerge <- function(){
  tmpdf = read.table("data\\ABS_data_by_DZN_2006\\AllCategories.txt", header=TRUE, na.strings="-", blank.lines.skip = TRUE, fill = TRUE)
  ...
  tmpdf = tmpdf[,!(colnames(tmpdf) %in% c("X...."))]
  ...
  empPolyRawDataFrame <- readOGR(dsn="data\\ABS_data_by_DZN_2006",layer="DZN_2006_VIC",encoding="utf8")
  ...
  writeOGR(obj=empPolyRawDataFrame, dsn="data\\ABS_data_by_DZN_2006", layer="DZN_X_Employment_2006_Orginal", driver="ESRI Shapefile", check_exists=TRUE, overwrite_layer=TRUE)
}
...
f_datamerge_2011_NSW <- function(){
  tmpdf = read.table("NSW\\2011\\NSW_INDP_X_DZN_2011_V2.txt", sep=",", header=TRUE, na.strings="-", blank.lines.skip = TRUE, fill = TRUE)
  # 2800 is somehow missing in the AllCategories.txt data but exists in the job categories. Have to append this column for integrity
  tmpdf[,"X2800"] = 0
  ...
  # join attribute data to polygon data on dzn code, all polygon data rows are kept
  empPolyRawDataFrame <- readOGR(dsn="NSW\\2011",layer="DZN_2011_NSW",encoding="utf8")
...
f_parse_divzonecode_fromnewdiv4 <- function(){
  # load div4 data from new csv file
  div4DF = read.table("VIC\\div4_v5.csv", header=TRUE, na.strings="NA", sep="\t", blank.lines.skip = TRUE, fill = TRUE, colClasses="character")
  # load div3-2-1 from old csv file, the column ZoneCodes will be updated with new div4 data
  div3DF = read.table("VIC\\div_v4\\div3_v4.csv", header=TRUE, na.strings="", sep="\t", blank.lines.skip = TRUE, fill = TRUE, colClasses="character")
  div2DF = read.table("VIC\\div_v4\\div2_v4.csv", header=TRUE, na.strings="", sep="\t", blank.lines.skip = TRUE, fill = TRUE, colClasses="character")
  div1DF = read.table("VIC\\div_v4\\div1_v4.csv", header=TRUE, na.strings="", sep="\t", blank.lines.skip = TRUE, fill = TRUE, colClasses="character")

  # rebuild zonecode string for div3-div2-div1
  div3DF[,"ZoneCodes"] = ""
  for(k in 1: nrow(div3DF)){
    div3Code = as.character(div3DF[k,"Div3Code"])
    filter = div4DF[,"Div3Code"] == div3Code
  ...
  write.table(div3DF, file="VIC\\div3_v5.csv", row.names = FALSE, sep="\t")
  ...
  write.table(div2DF, file="VIC\\div2_v5.csv", row.names = FALSE, sep="\t")
  ...
  write.table(div1DF, file="VIC\\div1_v5.csv", row.names = FALSE, sep="\t")
...
f_GenBaseMap_VIC_2006 <- function(){
  # algorithm start time
  sTime = Sys.time()
  
  # load shapefile in a much faster way, but without proj4string
  dznempDF <- readShapePoly("data\\ABS_data_by_DZN_2006\\DZN_X_Employment_2006_Orginal")
  splitDF <- readShapePoly("data\\ABS_data_by_DZN_2006\\plan_zone_fullcode_split_vic_2006")
  splitCentroidDF <- readShapePoints("data\\ABS_data_by_DZN_2006\\plan_zone_fullcode_split_centroid_vic_2006")
  ...
  for (i in 1:nrow(dznempDF)){ 
    ...
    #test if centroids are in current dzn polygon
    x = splitCentroidDF[dznArea,]
    containedIdx = as.integer(row.names(x))
    ...
    #offset 1 to get the real idx for datamat
    containedIdx = containedIdx + 1
  ...
  cols = colnames(tmpmx)
  numericCols = cols[!(c(1:length(cols)) %in% c(2,4,6,7))] # c(3,5,7,8,10)
  for(i in 1:length(numericCols)){
    print(sprintf("coverting numeric columns %i of %i", i, length(numericCols)))
    tmpmx[,numericCols[i]] = as.numeric(as.vector(tmpmx[,numericCols[i]]))
  }
  ...
  # setup proj info
  proj4string(splitDF)<-" +proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"
  
  writeOGR(obj=splitDF, dsn="outputs", layer="MidPoly_X_Employment_2006_VIC", driver="ESRI Shapefile", check_exists=TRUE, overwrite_layer=TRUE)
...
f_GenBaseMap_VIC_2011 <- function(){
  # algorithm start time
  sTime = Sys.time()
  
  # load shapefile in a much faster way, but without proj4string
  dznempDF <- readShapePoly("data\\geelong\\2011\\GL_2011_DZN_X_Employment")
  splitDF <- readShapePoly("data\\geelong\\2011\\GL_2011_Planzone")
  splitCentroidDF <- readShapePoints("data\\geelong\\2011\\GL_2011_Planzone_Centroid")
  ...
  cols = colnames(tmpmx)
  numericCols = cols[!(c(1:length(cols)) %in% c(3,5,7,8,10))] # c(3,5,7,8,10)
  for(i in 1:length(numericCols)){
    print(sprintf("coverting numeric columns %i of %i", i, length(numericCols)))
    tmpmx[,numericCols[i]] = as.numeric(as.vector(tmpmx[,numericCols[i]]))
  }
  ...
  writeOGR(obj=splitDF, dsn="outputs", layer="MidPoly_X_Employment_Geelong_2011", driver="ESRI Shapefile", check_exists=TRUE, overwrite_layer=TRUE)
...
f_GenBaseMap_VIC_2011_MeshBlock <- function(){
  # algorithm start time
  sTime = Sys.time()
  
  print("=== 1. start loading data ===")
  
  ZoneCodeColumnName = "MB_CAT11"
  
  # load shapefile in a much faster way, but without proj4string
  dznempDF <- readShapePoly("VIC\\2011\\DZN_X_Employment_2011_Orginal")
  splitDF <- readShapePoly("VIC\\2011\\mb_fullcode_split_vic_2011")
  splitCentroidDF <- readShapePoints("VIC\\2011\\mb_fullcode_split_centroid_vic_2011")
  ...
  for (i in 1:nrow(dznempDF)){ 
    ...
    # get non-duplicated zonecodes within that dnzarea
    zonecodes = levels(as.factor(tmpSplitDataMat[containedIdx,ZoneCodeColumnName]))
    ...
    for (jc in JobCats){
      ...
      for (cidx in containedIdx){      
        for (zc in zcs){
          matchrlt = regexpr(zc, tmpSplitDataMat[cidx,ZoneCodeColumnName], ignore.case=TRUE)
          if (attr(matchrlt,"match.length")==nchar(tmpSplitDataMat[cidx,ZoneCodeColumnName])){
            subContainedIdx = c(subContainedIdx, cidx)
            subContainedArea = c(subContainedArea, splitDF@polygons[[cidx]]@area)
            break
          }
  ...
  cols = colnames(tmpmx)
  numericCols = cols[!(c(1:length(cols)) %in% c(2:8, 11:25))] 
  for(i in 1:length(numericCols)){
    print(sprintf("coverting numeric columns %i of %i", i, length(numericCols)))
    tmpmx[,numericCols[i]] = as.numeric(as.vector(tmpmx[,numericCols[i]]))
  }
  
  print("=== 5. numeric column converted ===")
  ...
  writeOGR(obj=splitDF, dsn="basemaps\\vic", layer="MB_MidPoly_X_Employment_VIC_2011", driver="ESRI Shapefile", check_exists=TRUE, overwrite_layer=TRUE)
...
f_GenBaseMap_VIC_2006_MeshBlock <- function(){
  # algorithm start time
  sTime = Sys.time()
  
  print("=== 1. start loading data ===")
  
  ZoneCodeColumnName = "CATEGORY"
  
  # load shapefile in a much faster way, but without proj4string
  dznempDF <- readShapePoly("VIC\\2006\\DZN_X_Employment_2006_Orginal")
  splitDF <- readShapePoly("VIC\\2006\\mb_fullcode_split_vic_2006")
  splitCentroidDF <- readShapePoints("VIC\\2006\\mb_fullcode_split_centroid_vic_2006")
  ...
  cols = colnames(tmpmx)
  numericCols = cols[!(c(1:length(cols)) %in% c(3,6,11,12))] 
  for(i in 1:length(numericCols)){
    print(sprintf("coverting numeric columns %i of %i", i, length(numericCols)))
    tmpmx[,numericCols[i]] = as.numeric(as.vector(tmpmx[,numericCols[i]]))
  }
  ...
  writeOGR(obj=splitDF, dsn="basemaps\\vic", layer="MB_MidPoly_X_Employment_VIC_2006", driver="ESRI Shapefile", check_exists=TRUE, overwrite_layer=TRUE)
...
f_GenBaseMap_SA_2011_MeshBlock <- function(){
  # algorithm start time
  sTime = Sys.time()
  ...
  # load shapefile in a much faster way, but without proj4string
  dznempDF <- readShapePoly("SA\\2011\\DZN_X_Employment_2011_SA_Orginal")
  splitDF <- readShapePoly("SA\\2011\\mb_fullcode_split_sa_2011")
  splitCentroidDF <- readShapePoints("SA\\2011\\mb_fullcode_split_centroid_sa_2011")
  ...
  cols = colnames(tmpmx)
  numericCols = cols[!(c(1:length(cols)) %in% c(2:16, 19:25))] 
  for(i in 1:length(numericCols)){
    print(sprintf("coverting numeric columns %i of %i", i, length(numericCols)))
    tmpmx[,numericCols[i]] = as.numeric(as.vector(tmpmx[,numericCols[i]]))
  }
  ...
  writeOGR(obj=splitDF, dsn="basemaps\\sa", layer="MB_MidPoly_X_Employment_SA_2011", driver="ESRI Shapefile", check_exists=TRUE, overwrite_layer=TRUE)
...
f_GenBaseMap_NSW_2011_MeshBlock <- function(){
  # algorithm start time
  sTime = Sys.time()
  ...
  # load shapefile in a much faster way, but without proj4string
  dznempDF <- readShapePoly("NSW\\2011\\DZN_X_Employment_2011_NSW_Orginal")
  splitDF <- readShapePoly("NSW\\2011\\mb_fullcode_split_nsw_2011")
  splitCentroidDF <- readShapePoints("NSW\\2011\\mb_fullcode_split_centroid_nsw_2011")
  ...
  writeOGR(obj=splitDF, dsn="basemaps\\nsw", layer="MB_MidPoly_X_Employment_NSW_2011", driver="ESRI Shapefile", check_exists=TRUE, overwrite_layer=TRUE)
